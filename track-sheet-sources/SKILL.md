...
8. Default to `metadata_output=sidecar` when the caller is creating a MaybeAI workbook or worksheet for the product UI. In sidecar mode, do not create `source-tracking`, do not modify workbook styles, and do not write helper cells.
9. Use standalone worksheet mode only as a fallback when play-be metadata APIs are unavailable or the user explicitly asks for a workbook-visible audit worksheet.
...
- The chat response should be short and operational:
  ...
  - row count written to play-be sidecar or `source-tracking`
