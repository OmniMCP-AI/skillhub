...
- 经营驱动前台模块：当合同、用户、投放、云资源、HR、应收、回款、产品/客户等经营明细有可用行时，必须在 MaybeSheet 前台增加至少 1 个、通常 1-3 个 `经营驱动分析-*` 或等价经营驱动页；只把经营数据写进老板摘要、经营概览、风险建议、追问支持或底稿不满足要求
...
For financial statement analysis and management/operating analysis only, boss-review style MaybeSheet output MUST use the 9 front-sheet template published at `https://www.maybe.ai/docs/spreadsheets/d/6a1d15393638526b20e3b4df`. Each sheet has a fixed column schema; do not invent new column names. Every data row MUST carry a management/observation column (管理观察 / 管理说明 / 判断 / 建议 / etc.); bare numbers without management meaning are rejected by the output contract.

This template is scoped to 财报分析、三表分析、经营分析、管理分析、老板汇报 and equivalent single-company financial/management analysis requests. It is not a global finance-domain template and must not be used as the primary deliverable for consolidation reporting, budget variance analysis, cash/working-capital analysis, tax support, reconciliation, audit-only review, or other specialized finance tasks. Those request types must follow their own output contracts and quality gates.
...
When the product environment supports MaybeSheet, this template is the default only for financial statement analysis, three-statement analysis, management analysis, operating analysis, and boss-review analysis requests. The general 5-page structure described in §10 remains the audit/traceability/limitations/next-step frame for those scoped requests; the 9 front sheets here are the **content** the front of the report follows. The two layers compose: the 9 front sheets live on top, the audit / traceability / limitations / next-step pages live behind them.

This 10.3 section supersedes any softer instruction in the rest of this contract about default front-sheet naming only when the routed request type is `financial_report_analysis` or `management_analysis` and the user has not specified a different template. It must not supersede specialized output contracts such as consolidation reporting or budget variance analysis.
