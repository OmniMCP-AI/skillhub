...
## Default Output Shape for Financial/Management Reports (boss-review)

For financial statement analysis and management/operating analysis only (财务分析报告 / 财报分析 / 三表分析 / 经营分析 / 管理分析 / 老板汇报), the default output shape has two layers; deliver both unless the user explicitly asks for only one:
...
Scope boundary: this 9-sheet boss-review template is not a global finance-domain template. It must not be used as the primary deliverable for consolidation reporting, budget variance analysis, cash/working-capital analysis, tax support, reconciliation, audit-only review, or any other specialized finance request. Specialized request types must use their own Skill Pack output contract and quality gates.
...
- Load `references/slug-naming-migration.md` when updating FinClaw skill slugs, routing references, capability registries, or migrating old `fin-claw-*` runtime references to canonical `finclaw-*` names.
...
- Load `references/maybeai-delivery-readback.md` before claiming a MaybeSheet report delivery is complete; the skill's own delivery gate requires list_worksheets + read_sheet verification, and this reference ships the minimum-viable recipe.
